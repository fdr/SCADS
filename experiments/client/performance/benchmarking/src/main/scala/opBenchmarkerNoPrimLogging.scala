...
object BenchmarkOpsNoPrimLogging {
					...
					itemsRangeB.foreach((numB) => {
						...
						if (warmupDuration > 0 & !warmupDone) {
							...
							val threads = (1 to numThreads).toList.map((id) => {	
								val agent = new BenchmarkAgentNoPrimLogging(whichOp, s_to_ms(warmupDuration), aSize, bSize, numA, numB, maxItemsA, maxItemsB)
								new Thread(agent)
							})
						...
						val threads = (1 to numThreads).toList.map((id) => {	
							val agent = new BenchmarkAgentNoPrimLogging(whichOp, s_to_ms(runDuration), aSize, bSize, numA, numB, maxItemsA, maxItemsB)
							new Thread(agent)
						})
...
class BenchmarkAgentNoPrimLogging(opType:Int, duration_ms:Int, aSize:Int, bSize:Int, numA:Int, numB:Int, maxItemsA:Int, 
