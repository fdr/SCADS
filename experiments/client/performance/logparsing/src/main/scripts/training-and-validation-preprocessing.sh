...
java -DlogDir=$1/training-logs -cp $MY_SCADS/logparsing/target/logparsing-1.0-SNAPSHOT-jar-with-dependencies.jar parser.ParsePerThreadLogs

#for i in `jot $2 1 $2`;
for i in `seq 1 $2`
do echo Parsing validation $i per-thread logs...;
java -DlogDir=$1/validation-logs/validation$i-logs -cp $MY_SCADS/logparsing/target/logparsing-1.0-SNAPSHOT-jar-with-dependencies.jar parser.ParsePerThreadLogs;
done
