...
case class SCADrQueryMyFollowing (val username: String, val numUsersFollowed: Int)(implicit val env:Environment) extends SCADrQuery {
	def queryType: String = "myFollowing"
	
	def execute = {
		Queries.userByName(username).apply(0).myFollowing(numUsersFollowed)
	}
	
	override def toString: String = "myFollowing(" + username + "," + numUsersFollowed + ")"
}
...
class SimpleSCADrQueryGenerator (
	...
	def generateQuery: SCADrQuery = {
		...
		val numUsersToFollow = parameters("myFollowing")(0).toInt
		...
		mix.sampleQueryType match {
			...
			case "myFollowing" => new SCADrQueryMyFollowing(userByNameParamGenerator.generateParam, numUsersToFollow)
			/*
