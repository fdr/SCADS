...
object WorkloadRunner {
  	def main(args: Array[String]) {
		...
		*/
		val numUsersFollowed = System.getProperty("numUsersFollowed").toInt
		println("  numUsersFollowed=>" + System.getProperty("numUsersFollowed") + "<")
		...
		*/
		...
		var paramMap:Map[String, List[String]] = null

		
		//if (System.getProperty("numHashTagsPerThought") == null || System.getProperty("numDistinctHashTags") == null) {
			paramMap = getParamMap(numUsers, emailDupFactor, thoughtstreamLength, numUsersFollowed)
			populateDB(paramMap, emailDupFactor, thoughtsPerUser, subsPerUser)
		/*} else {
			paramMap = getParamMap(numUsers, emailDupFactor, thoughtstreamLength, numDistinctHashTags)
		...
		mixChoice match {
			...
			case "myFollowing" => mix = WorkloadGenerators.mixMyFollowing
	...
	def getParamMap(numUsers:Int, emailDupFactor:Int, numThoughts:Int, numUsersFollowed:Int): Map[String, List[String]] = {
		// Usernames
		...
		val paramMap = Map("userByName"->usernames, "userByEmail"->emails, "thoughtstream"->List(numThoughts.toString), "myFollowing"->List(numUsersFollowed.toString))	
		// "thoughtstream" and "myFollowing" reuse "usernames"
		paramMap
	}
